...
import {normalizeIndonesianPhone,phoneAliases} from '@/lib/phone';
import {createCustomerSession} from '@/lib/customer-auth';

const schema=z.object({
  serviceId:z.string().min(1),partnerId:z.string().min(1),areaId:z.string().min(1),
  name:z.string().trim().min(2).max(100),
  phone:z.string().transform((value,ctx)=>{try{return normalizeIndonesianPhone(value)}catch{ctx.addIssue({code:'custom',message:'Nomor WhatsApp tidak valid.'});return z.NEVER}}),
...
async function establishSession(publicId:string){
  const order=await db.order.findUnique({where:{publicId},select:{customer:{select:{id:true,user:{select:{id:true,sessionVersion:true}}}}}});
  if(order)await createCustomerSession({userId:order.customer.user.id,customerId:order.customer.id,sessionVersion:order.customer.user.sessionVersion});
}
...
export async function POST(req:NextRequest){
  try{
    ...
    if(existing?.response){const response=existing.response as {publicId?:string};if(response.publicId)await establishSession(response.publicId);return NextResponse.json(existing.response,{status:existing.statusCode||201})}
    const result=await db.$transaction(async tx=>{
      ...
      const unitPrice=selectedPartner.price,serviceSubtotal=calculateLineTotal({unitPrice,quantity:data.quantity,minimumCharge:service.minimumCharge}),areaSurcharge=area.surcharge,total=serviceSubtotal+areaSurcharge;
      ...
      let user=await tx.user.findFirst({where:{phone:{in:phoneAliases(data.phone)},role:'CUSTOMER'}});
      if(!user)user=await tx.user.create({data:{phone:data.phone,role:'CUSTOMER'}});
      else if(user.phone!==data.phone){const canonicalOwner=await tx.user.findUnique({where:{phone:data.phone},select:{id:true}});if(!canonicalOwner)user=await tx.user.update({where:{id:user.id},data:{phone:data.phone}})}
      ...
      else if((!customer.name.trim()||/^customer$/i.test(customer.name.trim()))&&customer.name!==data.name)customer=await tx.customerProfile.update({where:{id:customer.id},data:{name:data.name}});
      const existingAddress=await tx.address.findFirst({where:{customerId:customer.id,line1:data.address,areaId:data.areaId,phone:data.phone},select:{id:true}});
      if(!existingAddress)await tx.address.create({data:{customerId:customer.id,recipientName:customer.name,phone:data.phone,line1:data.address,areaId:data.areaId,latitude:data.latitude,longitude:data.longitude}});
    ...
    await establishSession(result.publicId);
